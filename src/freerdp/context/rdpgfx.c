...
static UINT rdpgfx_client_caps_advertise(RdpgfxServerContext *context, const RDPGFX_CAPS_ADVERTISE_PDU *capsAdvertise) {
    ...
    wsland_trace(FREERDP, INFO, "RDPGFX caps advertise: count=%u", capsAdvertise->capsSetCount);
    for (int i = 0; i < capsAdvertise->capsSetCount; i++) {
        ...
        wsland_trace(FREERDP, INFO, "RDPGFX caps[%d]: version=%u length=%u flags=0x%x",
            i, capsSet->version, capsSet->length, capsSet->flags);
        if (version_name) {
            wsland_trace(FREERDP, INFO, "RDPGFX caps[%d] version_name=%s", i, version_name);
        } else {
            wsland_trace(FREERDP, ERROR, "RDPGFX caps[%d] version_name=UNKNOWN(%u)", i, capsSet->version);
        }

        if (capsSet->flags & RDPGFX_CAPS_FLAG_THINCLIENT) {
            wsland_trace(FREERDP, INFO, "RDPGFX caps[%d] flag=THINCLIENT", i);
        }
        if (capsSet->flags & RDPGFX_CAPS_FLAG_SMALL_CACHE) {
            wsland_trace(FREERDP, INFO, "RDPGFX caps[%d] flag=SMALL_CACHE", i);
        }
        if (capsSet->flags & RDPGFX_CAPS_FLAG_AVC420_ENABLED) {
            wsland_trace(FREERDP, INFO, "RDPGFX caps[%d] flag=AVC420_ENABLED", i);
        }
        if (capsSet->flags & RDPGFX_CAPS_FLAG_AVC_DISABLED) {
            wsland_trace(FREERDP, INFO, "RDPGFX caps[%d] flag=AVC_DISABLED", i);
        }
        if (capsSet->flags & RDPGFX_CAPS_FLAG_AVC_THINCLIENT) {
            wsland_trace(FREERDP, INFO, "RDPGFX caps[%d] flag=AVC_THINCLIENT", i);
        }
    }
    ...
    wsland_trace(FREERDP, INFO, "RDPGFX caps confirm: version=%u flags=0x%x",
...
static UINT rdpgfx_client_frame_acknowledge(RdpgfxServerContext *context, const RDPGFX_FRAME_ACKNOWLEDGE_PDU *frameAcknowledge) {
    wsland_peer *peer = context->custom;
    wsland_trace(FREERDP, INFO, "RDPGFX frame ack: frame_id=%u current=%u acknowledged=%u",
